 ...
 *
 * <p>This interface is designed for zero-copy operation using {@link MemorySegment}. Implementations
...
public interface FramingHandler {
     ...
     * segment. The destination must have sufficient capacity to hold the entire frame
     * (header + payload).
     *
     ...
     * @return the total frame length (header + payload), which equals
     *         {@code sourceLength + getHeaderSize()}
     ...
     *
     * <p>This method reads the framing header to determine the payload length, validates the
     * frame, and copies the payload to the destination segment. If the source contains an
     * incomplete frame (fewer bytes than indicated by the header), this method returns -1.
     *
     * <h3>Return Values</h3>
     * <ul>
     ...
     *
     * <p>Attempts to frame messages larger than this size will result in a {@link FramingException}.
     * This limit is typically configured at construction time and may be constrained by protocol
     * requirements or memory considerations.
     *
